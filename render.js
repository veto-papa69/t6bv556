    ...
    if (callback_query && callback_query.data) {
      ...
      if (data.startsWith("admin_accept_") || data.startsWith("admin_decline_")) {
        ...
        try {
          ...
          if (memReq) { memReq.status = isAccept ? 'approved' : 'declined'; adminAccessRequests.set(requestId, memReq); }
          try { await AdminAccessRequest.findOneAndUpdate({ requestId }, { status: isAccept ? 'approved' : 'declined' }); console.log(`Admin ${requestId.slice(0,8)} ${isAccept ? 'APPROVED' : 'DECLINED'}`); } catch (e) { console.log(`Admin ${requestId.slice(0,8)} ${isAccept ? 'APPROVED' : 'DECLINED'} in memory`); }
          await fetch(`https://api.telegram.org/bot${botToken}/answerCallbackQuery`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ callback_query_id: callback_query.id, text: isAccept ? "Access Approved!" : "Access Declined!", show_alert: true })
          });
          ...
          await fetch(`https://api.telegram.org/bot${botToken}/editMessageText`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: callback_query.message.chat.id, message_id: callback_query.message.message_id, text: updatedText, parse_mode: 'Markdown' })
          });
        } catch (e) { console.error("Admin callback error:", e.message); }
...
app.post('/api/admin/request-access', async (req, res) => {
  try {
    ...
    const memReq = { id: requestId, ip: ip.toString(), status: 'pending', timestamp: Date.now() };
    ...
    try {
      ...
      await dbReq.save();
      console.log(`ADMIN REQUEST: ${requestId.slice(0,8)} from IP ${ip} - Saved to DB and Memory`);
    } catch (e) {
      console.log(`ADMIN REQUEST: ${requestId.slice(0,8)} saved to memory only - ${e.message}`);
    }
    res.json({ requestId, status: 'pending' });
    const botToken = process.env.TELEGRAM_BOT_TOKEN;
    const chatId = process.env.TELEGRAM_CHAT_ID;
    console.log(`Telegram check - Token: ${botToken ? 'YES' : 'NO'}, ChatID: ${chatId ? 'YES' : 'NO'}`);
    if (botToken && chatId) {
      (async () => {
        try {
          ...
          console.log(`Telegram admin result: ${result.ok ? 'SUCCESS' : 'FAILED'} - ${JSON.stringify(result).slice(0,300)}`);
        } catch (e) { 
          console.error("Telegram admin FAILED:", e.message);
        }
      })();
    }
  } catch (error) {
    console.error("Admin request error:", error);
    res.json({ requestId: Math.random().toString(36).substr(2, 9), status: 'pending' });
  }
});
...
app.get('/api/admin/check-access/:requestId', async (req, res) => {
  try {
    ...
    let reqData = adminAccessRequests.get(requestId);
    if (!reqData) {
      try { const dbReq = await AdminAccessRequest.findOne({ requestId }); if (dbReq) reqData = { status: dbReq.status }; } catch {}
    }
    if (!reqData) return res.status(404).json({ error: "Not found" });
    res.json({ status: reqData.status, requestId });
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    if (requestId) {
      let reqData = adminAccessRequests.get(requestId);
      if (!reqData) { try { const dbReq = await AdminAccessRequest.findOne({ requestId }); if (dbReq) reqData = { status: dbReq.status }; } catch {} }
      if (!reqData || reqData.status !== 'approved') return res.status(403).json({ error: "Not approved via Telegram" });
    }
...
app.get('/api/admin/check-session', async (req, res) => { try { res.json({ isAdmin: !!req.session?.isAdmin }); } catch { res.json({ isAdmin: false }); } });
app.post('/api/admin/logout', async (req, res) => { try { req.session.isAdmin = false; res.json({ success: true }); } catch { res.status(500).json({ error: "Failed" }); } });
app.get('/api/settings', async (req, res) => { try { res.json(adminSettings); } catch { res.status(500).json({ error: "Failed" }); } });
app.get('/api/settings/qr', async (req, res) => { try { res.json({ qrImageUrl: adminSettings.qrImageUrl, upiId: adminSettings.qrUpiId, instructions: adminSettings.qrInstructions }); } catch { res.status(500).json({ error: "Failed" }); } });
app.get('/api/settings/banner', async (req, res) => { try { res.json({ text: adminSettings.bannerText, enabled: adminSettings.bannerEnabled }); } catch { res.status(500).json({ error: "Failed" }); } });
app.post('/api/admin/settings/qr', async (req, res) => {
  try {
    if (!req.session?.isAdmin) { const token = req.headers['x-admin-token']; if (token !== (process.env.ADMIN_SECRET_TOKEN || "admin_secret")) return res.status(403).json({ error: "Admin required" }); }
...
app.post('/api/admin/settings/banner', async (req, res) => {
  try {
    if (!req.session?.isAdmin) { const token = req.headers['x-admin-token']; if (token !== (process.env.ADMIN_SECRET_TOKEN || "admin_secret")) return res.status(403).json({ error: "Admin required" }); }
