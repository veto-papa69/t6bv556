...
let adminSettings = {
  ...
  bannerText: process.env.DEFAULT_BANNER_TEXT || "इंस्टाबूस्ट प्रो - भारत का नंबर 1 SMM पैनल",
  bannerEnabled: true,
};
      ...
      if (data.startsWith("admin_accept_") || data.startsWith("admin_decline_")) {
        ...
        try {
          ...
          try {
            ...
            console.log(`Admin request ${requestId.slice(0,8)} ${isAccept ? 'APPROVED' : 'DECLINED'} in Memory`);
          }
          await fetch(`https://api.telegram.org/bot${botToken}/answerCallbackQuery`, {
            ...
            body: JSON.stringify({
              ...
              text: isAccept ? "Access Approved!" : "Access Declined!",
              show_alert: true
            })
          });
        } catch (e) {
  ...
  try {
    ...
    if (!reqData) {
      try {
        ...
        if (dbReq) reqData = { status: dbReq.status };
      } catch {}
    }
...
app.post('/api/admin/logout', async (req, res) => {
  try { req.session.isAdmin = false; res.json({ success: true }); } catch { res.status(500).json({ error: "Failed" }); }
});
...
app.post('/api/admin/settings/qr', async (req, res) => {
  try {
    if (!req.session?.isAdmin) {
      ...
      if (token !== (process.env.ADMIN_SECRET_TOKEN || "admin_secret")) return res.status(403).json({ error: "Admin required" });
    }
